    ...
    utility.ajax.helper(url, function (data) {
        ...
        $(data).each(function (ind, ele) {
            ...
            td = td + '<td><div class="btn-group" role="group" aria-label="Basic example">' +
                                '<button type="button" class="btn btn-secondary" data-id=' + ele["DepartmentId"] + ' onclick="department.edit(this)">Edit</button>' +
...
department.edit = function (row) {
    ...
    var preText = $(td).text();
    $(td).empty().append('<input type="text" class="form-control" value="' + preText + '"/>');
    $(row).parent().append('<button type="button" class="btn btn-secondary" onclick="department.cancelEdit('+row+')">cancel</button>');
...
department.cancelEdit = function (row) {
